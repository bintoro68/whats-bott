	...
	// Filter Banned People
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai minna~, Im Anker BOT. To find out the commands on this bot type ${prefix}menu`)
                      })
            } else {
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    aruga.reply(from, 'tuh sticker lu :v')
                ...
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ...
                    : aruga.reply(from, 'tuh sticker lu :v')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
