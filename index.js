...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('ANKER BOT', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('AnkeR', 'yellow'))
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hai minna~, Im Anker BOT. To find out the commands on this bot type ${prefix}menu`)
        })
	    }
	}
    })
